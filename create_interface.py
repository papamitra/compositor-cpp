...
def to_cpp_type(ns, arg):
    ...
    if (arg.attrib['type'] == 'object'):
        ...
        return 'class ' + to_class_name(ns, arg.attrib['interface']) + '*'

def to_class_name(ns, name):
    name = re.sub(ns + '_', '', name)
    return ''.join(map(lambda s: s.title(), name.split('_')))

def to_member_funcs(ns, reqs):
    s = ""
    for (name, args) in reqs:
        s += '\n'
        s += to_member_func(ns, name, args)
    return s

def to_member_func(ns, name, args):
    s = '    virtual void ' + name + '('
    s += '\n        Client& client'
    s += ',\n        Resource& resource'
    for arg in args:
        s += ',\n        ' + to_cpp_type(ns, arg) + ' ' + arg.attrib['name']
    s += ') = 0;\n'
    return s

def to_static_funcs(ns, ifname, reqs):
    s = ""
    for (name, args) in reqs:
        s += '\n'
        s += to_static_func(ns, ifname, name, args)
    return s

def to_static_func(ns, ifname, name, args):
    classname = to_class_name(ns, ifname) + 'Interface'
...
def create_interface_instance(ns, name, reqs):
    if len(reqs) == 0: return ""
    s = 'struct {ifname}_interface {ifname}_interface_instance = {{'.format(ifname=name)
    for (funcname, args) in reqs:
        s += '\n    &{classname}Interface::{funcname}_,'.format(classname=to_class_name(ns, name), funcname=funcname)
    s += '\n};'
    return s
...
def create_interface(ns, intf):
    (name, reqs) = intf
    s = ""
    s += class_format.format(classname=to_class_name(ns, name),
                             member_funcs=to_member_funcs(ns, reqs),
                             static_funcs=to_static_funcs(ns, name, reqs),
...
def main():
    ...
    for child in root:
        if child.tag == 'interface' and child.attrib['name'] not in exclude_interface:
            ...
            header_str += '\n' + create_interface(ns, interface)
            source_str += '\n' + create_interface_instance(ns, *interface)

    print(header_preamble.format(namespace=ns, interfaces=header_str), file=header_file)

    print(source_preamble.format(namespace=ns, interface_instance=source_str), file=source_file)
