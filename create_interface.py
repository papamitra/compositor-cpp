...
static_function = """
    static void {reqname}_(
        ...
        WlClient client_(client);
        WlResource resource_(resource);{to_class}
...
def to_cpp_type(arg):
    ...
    if t: return t
    if (arg.attrib['type'] == 'object'):
        cname = to_class_name(arg.attrib['interface'])
        return 'class {cname}*'.format(cname=cname)
...
def to_member_event_func(ifname, name, args):
    s = '    void send_' + name + '(WlResource& resource_'
...
def to_static_func(ifname, name, args):
    ...
    for arg in args:
        if arg.attrib['type'] == 'object' and arg.attrib['interface'] not in acceptable_interface:
            cname = to_class_name(arg.attrib['interface'])
            to_class += '\n        auto {name}_ = ({cname}*)wl_resource_get_user_data((wl_resource*)({name}));'.format(cname=cname, name=arg.attrib['name'])
